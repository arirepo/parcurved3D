...
c$$$C
c$$$C
c$$$C   THIS DRIVER DEMONSTRATES SOFTWARE PACKAGES TRIPAK AND
c$$$C SRFPAK BY CREATING A TRIANGULATION OF EIGHT NODES WHOSE
c$$$C CONVEX HULL IS THE UNIT SQUARE, AND INTERPOLATING A SET
c$$$C OF DATA VALUES AT THE VERTICES TO A UNIFORM GRID OF 20
c$$$C INTERPOLATION POINTS.  THE TRIANGULATION DATA STRUCTURE
c$$$C IS PRINTED, FOLLOWED BY THE MAXIMUM INTERPOLATION ERROR.
c$$$C THIS SHOULD BE CLOSE TO ZERO SINCE THE DATA VALUES ARE
c$$$C TAKEN FROM A QUADRATIC FUNCTION FOR WHICH THE INTERPOLA-
c$$$C TION METHOD IS EXACT.
c$$$C
c$$$      INTEGER IADJ(48), IEND(8)
c$$$      REAL    X(8), Y(8), Z(8), ZXZY(2,8),
c$$$     .        PX(5), PY(4), ZZ(5,4)
c$$$      DATA    N/8/,  LUN/6/,  NROW/5/,  NI/5/,  NJ/4/
c$$$      DATA   X(1), X(2), X(3), X(4), X(5), X(6), X(7), X(8)
c$$$     .       /0.,   1.,   1.,   0.,  .25,   .5,  .75,  .5/,
c$$$     .       Y(1), Y(2), Y(3), Y(4), Y(5), Y(6), Y(7), Y(8)
c$$$     .       /0.,   0.,   1.,   1.,   .5,   .3,   .5,  .7/
c$$$      DATA   PX(1), PX(2), PX(3), PX(4), PX(5)
c$$$     .       /.1,    .3,    .5,    .7,    .9/,
c$$$     .       PY(1), PY(2), PY(3), PY(4)
c$$$     .       /.2,    .4,    .6,    .8/
c$$$      F(XF,YF) = (XF+YF)**2
c$$$C
c$$$C COMPUTE DATA VALUES
c$$$C
c$$$      DO 1 K = 1,N
c$$$    1   Z(K) = F(X(K),Y(K))
c$$$C
c$$$C REORDER THE NODES AND DATA VALUES BY SORTING ON
c$$$C   X-COMPONENTS.  IEND IS USED AS TEMPORARY STORAGE
c$$$C   FOR A PERMUTATION VECTOR.
c$$$C
c$$$      CALL REORDR (N,3, X,Y,Z, IEND)
c$$$C
c$$$C CREATE THE TRIANGULATION.  THE ERROR FLAG IS IGNORED
c$$$C   SINCE, AFTER ALL, NOTHING CAN GO WRONG.
c$$$C
c$$$      CALL TRMESH (N,X,Y, IADJ,IEND,IER)
c$$$C
c$$$C PRINT NODAL COORDINATES AND ADJACENCY INFORMATION ON
c$$$C   UNIT LUN.
c$$$C
c$$$      CALL TRPRNT (N,LUN,X,Y,IADJ,IEND,0)
c$$$C
c$$$C INTERPOLATE TO THE UNIFORM GRID POINTS.  THE FLAG
c$$$C   SPECIFIES THAT DERIVATIVES ARE TO BE COMPUTED
c$$$C   INTERNALLY AND SAVED IN ZXZY.
c$$$C
c$$$      IFLAG = 2
c$$$      CALL UNIF (N,X,Y,Z,IADJ,IEND,NROW,NI,NJ,PX,PY,
c$$$     .           IFLAG, ZXZY,ZZ,IER)
c$$$C
c$$$C COMPUTE AND PRINT THE MAXIMUM INTERPOLATION ERROR.
c$$$C
c$$$      ERMAX = 0.
c$$$      DO 2 J = 1,NJ
c$$$        DO 2 I = 1,NI
c$$$          ERR = F(PX(I),PY(J)) - ZZ(I,J)
c$$$    2     ERMAX = AMAX1(ABS(ERR),ERMAX)
c$$$      WRITE (LUN,100) ERMAX
c$$$  100 FORMAT (//1H ,30HMAXIMUM INTERPOLATION ERROR = ,
c$$$     .        E10.3)
c$$$      STOP
c$$$      END

      SUBROUTINE COMPNTRI (N,LUNIT,IADJ,IEND,IFLAG, NTRIOUT)
      INTEGER N, LUNIT, IADJ(1), IEND(N), IFLAG, NTRIOUT
...
c$$$      WRITE (LUN,101)
      ...
      DO 1 NODE = 1,NN
        ...
        INC = (INDL - INDF)/14 + 2
        NL = NL + INC
c$$$        IF (NL .GT. NLMAX) WRITE (LUN,106)
        IF (NL .GT. NLMAX) NL = INC
c$$$        WRITE (LUN,103) NODE, (IADJ(I), I = INDF,INDL)
c$$$        IF (INDL-INDF .NE. 13) WRITE (LUN,105)
        INDF = INDL + 1
    1   CONTINUE
      GO TO 4
C
...
c$$$     .                  (IADJ(I), I = INDF,INDL)
c$$$        IF (INDL-INDF .NE. 7) WRITE (LUN,105)
        INDF = INDL + 1
    3   CONTINUE
C
C PRINT NB, NA, AND NT
